...
Второе ограничение — приватность. Главная страница обещает ноль трекеров,
значит, считать посещаемость в браузере нельзя. Она считается из отдельного
журнала nginx, в котором физически нет ни IP, ни User-Agent, ни Referer, ни
строки запроса: процесс, который никогда не видел персональных данных, не
может их случайно опубликовать.
...
| nginxlog_exporter | v1.11.0 | посещаемость сайтов из журнала nginx |
