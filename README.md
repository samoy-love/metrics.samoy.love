...
| node_exporter на хосте | 1.10.2 (пакет) | температуры, вентиляторы, ZFS, SMART, шейпер — то, чего не видно из контейнера |
| метрики Incus | встроены в Incus 7.4 | CPU, память, диск и сеть по каждому контейнеру и виртуалке |
...
## Метрики хоста: почему экспортёров два

Стек целиком живёт в контейнере `web`, и `node_exporter` из этого
`docker-compose.yml` видит только его. Раньше это и был весь сервер — на
прежней виртуалке физических датчиков не существовало, поэтому в его команде
стоит `--no-collector.hwmon`. После переезда на своё железо контейнер остался
контейнером, а температуры, обороты вентиляторов, ZFS, SMART и классы шейпера
появились уровнем ниже, на хосте.

Поэтому на самом хосте стоит **второй** `node_exporter`, пакетом
(`prometheus-node-exporter`), и отдаёт то, чего первый не видит:

* температуры CPU, обоих модулей памяти, обеих NVMe и iGPU;
* обороты вентиляторов и уровни PWM — через модуль `nct6775` (чип NCT6799),
  который закреплён в `/etc/modules-load.d/`, иначе датчики исчезают после
  перезагрузки;
* SMART обоих дисков и здоровье пула ZFS — их не отдаёт ни один коллектор,
  поэтому `/usr/local/sbin/node-textfile-collect` раз в пять минут пишет
  `.prom` в textfile-каталог;
* очередь заданий GitHub Actions на своих раннерах (Linux и win-builder) —
  `/usr/local/sbin/ci-queue-collect` раз в 30 секунд опрашивает GitHub тем же
  токеном, что и раннеры, и пишет `ci.prom`; исходник и юниты — в
  репозитории home-server, панель — «CI — раннеры и очередь»;
* классы шейпера `wan-shaper` (коллектор `qdisc`) и состояние хостовых
  юнитов — incus, три таблицы изоляции, сторож Windows-машины.

Слушает он только на внутреннем мосту, наружу порт не открыт. Prometheus
дотягивается до него точечным разрешением в таблице `web_isolation` на хосте;
всё остальное контейнеру там по-прежнему закрыто.

Отдельная цель `incus` — встроенный экспортёр самого Incus: CPU, память, диск
и сеть по каждому инстансу, без установки чего-либо внутрь. Сертификат сервера
Incus выписан на имя хоста, среди SAN адреса моста нет, поэтому имя для
проверки TLS задаётся отдельным `server_name`, а не подменой DNS. Клиентский
сертификат зарегистрирован с типом `metrics`: он даёт доступ только к
`/1.0/metrics` и не позволяет управлять инстансами; ключ лежит вне
репозитория, в `/etc/samoylove-metrics/incus`.

Адреса обеих целей тоже вынесены из репозитория — в `file_sd`-файлы
`/etc/samoylove-metrics/targets/`. Репозиторий публичный, а адреса зависят от
того, как собрана конкретная установка.

Единственное, чего эта цель не показывает, — что происходит **внутри**
Windows-машины: детальные метрики гостя Incus собирает через `incus-agent`, а
он существует только для Linux.

Панели по всему этому — в дашборде «Сервер — железо и инфраструктура»
(`grafana/dashboards/hardware.json`), пороги алертов — в группе
`infra-hardware`.
...
**Группа `product-cs2` — по-русски целиком, и это единственное исключение.**
Решение владельца от 10.09.2026: имя правила и его текст читает человек в чате
ночью, и читать он должен не `Cs2ChainStalled`, а что сломалось и чего от этого
не работает у игрока. У каждого правила там два текста: `summary` — что
случилось, `description` — что это значит для человека и что делать. Второе
важнее первого: тревога, из которой не видно последствия, требует похода в код,
чтобы понять, надо ли вставать, — а тревога, по которой не встают, через месяц
выключается вместе со всеми остальными. Остальные группы остались на английском,
потому что переводить их без нужды значит на день сделать нечитаемыми обе
половины.
