...
Адреса этих целей тоже вынесены из репозитория — в `file_sd`-файлы
`/etc/samoylove-metrics/targets/`. Репозиторий публичный, а адреса зависят от
того, как собрана конкретная установка.

Там же лежит `cs2.yml` — цель разбора матчей CS2. Сервис живёт в отдельном
контейнере, поэтому `host.docker.internal` для него не годится: у контейнера
свой сетевой namespace и свой адрес. Метрики он отдаёт на отдельном порту по
`/internal/metrics`, а не через nginx, чтобы продуктовые числа нельзя было
открыть наружу забытым location.

**Файл цели кладётся руками один раз, и без него сбор молча прекращается.**
Именно так и вышло: цель `cs2-api` вместе с пробой `cs.samoy.love` жила только
в конфиге на сервере, а в репозиторий не попала — то есть следующая же выкатка
мониторинга стёрла бы её, и никто бы этого не заметил. Теперь цель описана
здесь, а `file_sd` остаётся ровно тем, ради чего он и заведён: адресом, а не
самой целью.
