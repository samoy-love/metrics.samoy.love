...
- **Посещаемость сайтов** — все пять доменов: samoy.love, metro, snakes,
  status и launcher. Запросы по домену и пути,
...
- **Snakes** — игровые метрики: люди и боты отдельными счётчиками, заходы,
  длительность матча и число участников на старте, причины смерти, захват
  клеток и убийства, подобранные бонусы, контракты и мутаторы. Отдельно —
  здоровье реального времени: длительность игрового такта, обрывы WebSocket
  по причине, потерянные сообщения, отбитые рукопожатия.
...
(посещаемость, лаунчер, snakes, метро, состояние статус-страницы).
...
`IPAddressAllow=localhost` и `IPAddressDeny=any`, то есть ядро режет любое
подключение не с `127.0.0.1`. Prometheus сидит в своём сетевом namespace и
приходит с адреса docker-моста, поэтому вместо метрик получал таймаут —
причём молча, пакеты гасит ядро, и в журнале сервиса нет ни строчки. Обходить
намеренное ограничение чужого сервиса из мониторинга мы не стали: весь его
смысл в том, что игровой процесс виден только nginx'у на этой же машине.
Развязали на стороне snakes и в две части: drop-in к юниту открывает ровно
подсети docker-мостов, а игра поднимает на адресе моста отдельный слушатель,
отдающий только `/metrics`, — иначе вместе с ним контейнерам открылись бы
`/ws` и статика мимо nginx. Цель с тех пор `up`. По той же причине сервисы
...
| [snakes](https://github.com/tr0llex/snakes) | игра; метрики отдаёт отдельным слушателем на адресе моста (см. ловушки) |
